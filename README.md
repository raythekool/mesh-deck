...
- **Node sidebar**: A dockable, mouse-clickable list of known nodes stays visible next to the console; click a row (or select it and press `Enter`) to open that node's detail. Toggle it with `Ctrl+B`; the state is remembered across restarts.
